...
class RuleEngine:
    ...
    def _discover_rules(self) -> List[BaseRule]:
        ...
        for _, module_name, is_pkg in pkgutil.walk_packages(pkg.__path__, pkg.__name__ + "."):
            if not is_pkg:
                try:
                    ...
                    print(f"[Warning] Failed to import rule module {module_name}: {e}", file=sys.stderr)
        ...
        for rule_cls in rule_classes:
            ...
            if getattr(rule_cls, "RULE_ID", None) and not getattr(rule_cls, "__abstractmethods__", None):
                try:
                    discovered.append(rule_cls())
                except Exception as e:
                    print(f"[Error] Failed to instantiate rule class {rule_cls.__name__}: {e}", file=sys.stderr)
        ...
        for rule in self.rules:
            ...
            if "*" in rule.TARGET_NODE_TYPES or (node_type and node_type in rule.TARGET_NODE_TYPES):
                try:
                    ...
                    print(f"[Error] Exception raised in rule {rule.RULE_ID} ({rule.NAME}): {e}", file=sys.stderr)
